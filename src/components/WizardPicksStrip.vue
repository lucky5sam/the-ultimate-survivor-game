...
// roster slots (empty = bordered circle with the slot number). The MVP slot gets
// a gold ring + crown. Once a bounty is chosen, a divider and a parchment scroll
// with the pick's preferred name appear after the roster. A trailing label is
    ...
    <div class="flex min-w-0 flex-col items-start">
      ...
      <div class="flex items-center gap-x-2">
        ...
        <div v-for="i in 4" :key="i" class="flex w-11 flex-col items-center gap-0.5">
          <template v-if="picks[i - 1]">
            <div class="relative">
              ...
              >
                ...
                >
                  <i class="fa-solid fa-user text-base text-icon-subtle"></i>
                </div>
              </div>
          ...
          <template v-else>
            <div
              class="flex h-9 w-9 items-center justify-center rounded-full border-2 border-border-subtle bg-surface-subtle/40"
            >
        ...
        <!-- Divider + bounty parchment: only once a bounty is chosen -->
        <template v-if="bounty">
          <div class="mx-1 h-9 w-px shrink-0 bg-border-subtle" />

          <!-- Bounty slot: the pick's preferred name on a parchment scroll -->
          <div class="relative w-20 shrink-0">
            <img
              :src="parchmentUrl"
              alt=""
              aria-hidden="true"
              class="w-full select-none [filter:drop-shadow(0_2px_4px_rgba(0,0,0,0.35))]"
            />
            <span
